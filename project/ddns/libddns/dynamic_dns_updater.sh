...
[ $ERR_LAST -ne 0 ] && {
	...
	# write_log  7 "uci configuration skipped (HE config)"
...
[ -n "$service_name" ] && {
	...
	get_service_data "$service_name" "${SHAREDDNS}/custom" update_url update_script UPD_ANSWER
	if [ "$?" != "0" ]; then
		get_service_data "$service_name" "${SHAREDDNS}/default" update_url update_script UPD_ANSWER
	fi
}
...
# fill lookup_host from domain when empty
[ -z "$lookup_host" ] && lookup_host="$domain"
...
# Skinos status for ddns@scripts.status (matches var2path on device)
mkdir -p "$DDNS_STATUS_DIR"
echo "{ \"state\":\"register\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
...
# Skinos status
echo "{ \"state\":\"register\",\"real_ip\":\"$REGISTERED_IP\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
...
while : ; do
	...
	echo "{ \"state\":\"register\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${CURRENT_IP}\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
	...
	if [ $CURR_TIME -ge $NEXT_TIME -o "$CURRENT_IP" != "$REGISTERED_IP" ]; then
		...
		if [ $ERR_LAST -eq 0 ]; then
			...
			echo "{ \"state\":\"succeed\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${CURRENT_IP}\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
			...
			echo "{ \"state\":\"already\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${CURRENT_IP}\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
		else
			write_log 3 "IP update not accepted by DDNS Provider"
			echo "{ \"state\":\"deny\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${CURRENT_IP}\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
		fi
	else
		echo "{ \"state\":\"noneed\",\"domain_ip\":\"$REGISTERED_IP\",\"extern_ip\":\"${CURRENT_IP}\" }" > "$DDNS_STATUS_DIR/$SECTION_ID"
	fi
