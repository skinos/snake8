...
sends `AT+CFUN=0` on the AT port (`stty`), then starts QLog on `dtty`. After capture
starts it sends `AT+CFUN=1`, waits 2 seconds, then `modem@….setup` and
...
- Description: Capture on `modem@lte` / `modem@lte2`. Port from `modem@….tty:dtty`; AT on `….tty:stty`. Sequence: `service.stop[ network@connect ]` → `ifname@….shut` → `modem@….shut` → `AT+CFUN=0` → start QLog → `AT+CFUN=1` → wait 2s → `modem@….setup` → `ifname@….setup`. Re-call replaces the previous capture.
