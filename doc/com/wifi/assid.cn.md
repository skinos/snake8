## wifi@assid — 5.8G SSID 管理
管理 5.8G SSID
通常 wifi@assid 是第一个 5.8G SSID。如果系统中有多个 5.8G SSID, wifi@assid2 将是第二个 5.8G SSID, 依此递增

### 配置 ( `wifi@assid` )
**wifi@assid** 是第一个 5.8G SSID   
**wifi@assid2** 是第二个 5.8G SSID   
...
示例, 显示第一个 5.8G SSID 所有配置
```shell
wifi@assid
{
    "status":"enable",              # 启用 SSID
    "ssid":"5228-test-5.8g",        # SSID 名称为 5228-test-5.8g
...
示例, 修改第一个 5.8G SSID 名称为 myNewSSID
```shell
...
示例, 修改第一个 5.8G SSID 安全模式为 WPAPSK 并将 WPA 密钥改为 88888888
```shell
wifi@assid:secure=wpapsk            # 修改安全模式为 WPAPSK
ttrue
wifi@assid:wpa_encrypt=tkipaes      # 修改 WPA 加密为自动
ttrue
wifi@assid:wpa_key=88888888         # 修改 WPA 加密密钥为 88888888
ttrue
...
### 组件 API
**可直接调用** 的 API: `wifi@assid.method`, `wifi@assid2.method`, ... (HE / eline / HTTP `/he`).
...
+ `stabeat[ MAC address ]` **断开客户端连接**   
    ...
    示例, 从第一个 5.8G SSID 断开客户端 00:03:7F:13:BD:30
    ```shell
