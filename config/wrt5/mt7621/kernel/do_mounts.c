...
static void __init mount_fail_erase_kernel_partition(void)
{
	struct mtd_info * mtd;
	...
	if (!mtd) {
		printk(KERN_ERR "Can't find mtd ubi\n");
		return;
	}
	instr.addr = 0;
	instr.len = 4 * mtd->erasesize;

	printk(KERN_ALERT "erase firmware mtd block\n");
	mtd_erase(mtd, &instr);
}
...
retry:
	...
	printk("\n");
	//add by qingcheng
	mount_fail_erase_kernel_partition();
