...
/*
	//modify by qingcheng we will request irq base gpiod to irq
	if (!spi->irq)
...
*/
	...
	if (!spi->irq) {
		spi->irq = gpiod_to_irq(rx_int);
		if (!spi->irq) {
			return dev_err_probe(&spi->dev, -ENXIO,
				     "No IRQ specified \" in DT missing)!\n");
		}
		printk(KERN_ALERT "request mcp251xfd irq is %d\n", spi->irq);
	}
