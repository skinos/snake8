...
static int mtk_mii_ioctl(struct mii_bus *bus, struct ifreq *ifr, int cmd)
{
	struct mii_ioctl_data *mii = if_mii(ifr);
	int prtad, devad;
	unsigned int phy_id, reg;
	int ret = -EOPNOTSUPP;
	...
	case SIOCGMIIREG:
		if (mdio_phy_id_is_c45(phy_id)) {
			...
			//printk("mtk mii ioctl c45 %u %u\n", phy_id, reg);		
			ret = mdiobus_c45_read(bus, prtad, devad,
					reg);
		} else {
			//printk("mtk mii ioctl c22 %u %u\n", phy_id, reg);		
			ret =  mdiobus_read(bus, phy_id, reg);
		}
	...
	case SIOCSMIIREG:
		if (mdio_phy_id_is_c45(phy_id)) {
			...
			//printk("mtk mii s ioctl c45 %u %u\n", phy_id, reg);		
			ret =  mdiobus_c45_write(bus, prtad, devad,
					 reg, mii->val_in);
		} else {
			//printk("mtk mii s ioctl c22 %u %u\n", phy_id, reg);		
			ret = mdiobus_write(bus, phy_id, reg, mii->val_in);
		}
		break;
...
static int mtk_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	//add by qingcheng 2026.05.28
	eth = mac->hw;
	if (!eth && !eth->mii_bus) {
		printk(KERN_ERR "mtk_do_ioctl phy_bus unsupport\n");
		return -EOPNOTSUPP;
	}
	...
	case SIOCGMIIREG:
	case SIOCSMIIREG:
		//del by qingcheng return phylink_mii_ioctl(mac->phylink, ifr, cmd);
		return mtk_mii_ioctl(eth->mii_bus, ifr, cmd);
	default:
		break;
	}
