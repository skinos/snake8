...
define Build/Prepare/Default
	...
	# do not ship local IDE database file
	rm -f $(PKG_BUILD_DIR)/compile_commands.json
endef
...
#     仅当$(PKG_BUILD_DIR)下对应名为目录时才执行make; 预置脚本/二进制等非目录项跳过(与Build/Install/fpk一致)
...
define Build/Compile/ko
	+$(MAKE_VARS); \
	for i in $(1) ;do \
		$(MAKE) LINUX_DIR=$(LINUX_DIR) STAGING_DIR=$(STAGING_DIR)  -C "$(LINUX_DIR)" $(KERNEL_MAKE_FLAGS) M="$(PKG_BUILD_DIR)/$$$$i" EXTRA_CFLAGS="$(BUILDFLAGS)" modules; \
	done
endef
# 定义项目编译函数
